...
export async function rateAndPredictLeague(db: PrismaClient, leagueId: string, opts: { now?: Date; newsLoader?: NewsLoader } = {}) {
  ...
  const hist = await db.fixture.findMany({
    where: { leagueId, status: "FINISHED", homeGoals: { not: null }, kickoffUtc: { gte: new Date(now.getTime() - 450 * DAY), lt: now } },
    orderBy: { kickoffUtc: "asc" },
  });
  const ms: HistMatch[] = hist.map((f) => ({
    homeId: f.homeTeamId, awayId: f.awayTeamId, date: f.kickoffUtc, homeGoals: f.homeGoals!, awayGoals: f.awayGoals!,
    homeXg: f.homeXg, awayXg: f.awayXg, homeShots: f.homeShots, awayShots: f.awayShots,
  }));
  const fit = fitLeague(ms, now);
  ...
  if (fit.teams.size) {
    await db.teamRatingSnapshot.createMany({
      data: [...fit.teams].map(([teamId, r]) => ({ teamId, asOf: now, modelVersion: MODEL_VERSION, inputKind: fit.inputKind, attack: r.attack, defence: r.defence, sampleWeight: r.sampleWeight, matches: r.matches })),
    });
  }
  ...
  const lastPlayed = (teamId: string, before: Date) =>
    [...hist].reverse().find((f) => (f.homeTeamId === teamId || f.awayTeamId === teamId) && f.kickoffUtc < before);
  const form = (teamId: string, before: Date) =>
    [...hist].reverse().filter((f) => (f.homeTeamId === teamId || f.awayTeamId === teamId) && f.kickoffUtc < before).slice(0, 5)
      .map((f) => { const gf = f.homeTeamId === teamId ? f.homeGoals! : f.awayGoals!; const ga = f.homeTeamId === teamId ? f.awayGoals! : f.homeGoals!; return gf > ga ? "W" : gf === ga ? "D" : "L"; }).join("");

  let written = 0;
  for (const fx of upcoming) {
    const prev = fx.predictions[0];
    if (prev?.lockedAt) continue; // locked snapshots are immutable; post-lock revisions come from the lineup job (Phase 3)
    const lh = lastPlayed(fx.homeTeamId, fx.kickoffUtc), la = lastPlayed(fx.awayTeamId, fx.kickoffUtc);
    ...
    const out = predictFixture({
      fit, homeId: fx.homeTeamId, awayId: fx.awayTeamId, homeName: fx.homeTeam.shortName ?? fx.homeTeam.name, awayName: fx.awayTeam.shortName ?? fx.awayTeam.name,
      ...
      formHome: form(fx.homeTeamId, fx.kickoffUtc), formAway: form(fx.awayTeamId, fx.kickoffUtc),
      calibrators: set, calibrationResidual: residual,
    });
  ...
  return { fitted: fit.teams.size, inputKind: fit.inputKind, predictions: written };
}
